...
let TopicService = class TopicService {
    constructor(repo, chapterService, variableService) {
        this.repo = repo;
    ...
    async findAll(userId) {
        return this.repo
            ...
            .leftJoin('c.variables', 'a')
            .leftJoin('t.userTopic', 'ut', 'ut.userId = :userId', { userId })
            .select([
            't.*',
            'COUNT(DISTINCT c.id) AS chapterCount',
            'COUNT(a.id) AS variableCount',
            'ut.success as success',
            'ut.active as active',
        ])
            .groupBy('t.id')
            .getRawMany();
